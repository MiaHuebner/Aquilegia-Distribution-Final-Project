...
In this code block, make sure to **change all pathways to a place in your computer** :
...
```{r}
r_pred_fut <- rast(bio_fut_df[, -c(3:5)])

plot(r_pred_fut[[-1]])
```

Further break down the future predictions into looking at the predictions in the climates projected to remain the same:
...
Bio_analog <- rast(bio_analog_df)

plot(Bio_analog)
```

The RF model, compared to the GLM, shows more projected change in the range of *Aquilegia formosa* in the 2021-2040 time period with the projected climatic shifts. It shows a lower projected distribution of *Aquilegia formosa* found near California and up north in Alaska and western Canada. With the color differences, however, the scales must be noted. Overall, the above two maps show that based on both the GLM and RF models, the distribution of *Aquilegia formosa* will likely remain the same in the analogous climates, with a possible difference in the northern-most and southern-most regions of this species range.

Now, let's look at the novel climates:

```{r}
bio_novel_df <- bio_fut_df[, c('x', 'y', 'pred_glm', 'pred_rf')]

bio_novel_df[bio_fut_df$eo_mask == 0, 
             c('pred_glm', 'pred_rf')] <- NA

Bio_novel <- rast(bio_novel_df)

plot(Bio_novel)
```

As there are very few novel environments projected for the 2021-2040 time period under the Canadian Earth Systems Model with moderate emission guidelines set by societies, *Aquilegia formosa* is not likely to disperse to many novel environments in the 2021-2040 time period.

Overall, based on 2023-2024 distribution data on *Aquilegia formosa*, from 2021-2040 under moderate emissions using the Canadian Earth Systems Model as predicted future climatic data, the Generalized Linear Model and Random Forest models suggest the distribution of *Aquilegia formosa* will not significantly shift, nor will the future climatic conditions result in novel environments. During analysis, however, the Generalized Linear Model did not fit the data well, or did not converge. Many different strategies were employed in attempt to fix this. First, different combinations of bioclimatic data were used. After this failed, the model was simplified by removing the square term from each variable. When this also failed, the "brglm2" package was used. This generated a model that did converge, however, upon further inspection, the partial response curves yielded little information on how *Aquilegia formosa* interacts with different environmental conditions. Due to this, the non-convergent GLM model was chosen for all further analysis. Similarly to the GLM, the Random Forest model did not fit the data well, producing warnings saying "fewer than 5 unique values were found". The porr-fit of the models is highlighted when performing the model assessment. For both the Random Forest and GLM models, the area under the curve (AUC), true skill statistic (TSS), and other measures were all 1. These values lack strength.

In future analyses, different Species Distribution Models should be investigated to determine which fits the data better. Some future models could include a simple Logistic Regression, Generalized Additive Model, or a more complex MaxEnt analysis. Future analyses may also compare previous year/decade distribution data with the current distribution data, checking for a significant difference in range between past and current data as well.

#### References:

Bhat, I. A., Fayaz, M., Roof-ul-Qadir, Rafiq, S., Guleria, K., Qadir, J., wani, T. A., & Kaloo, Z. A. (2023). Predicting potential distribution and range dynamics of Aquilegia fragrans under climate change: Insights from Ensemble Species Distribution Modelling. *Environmental Monitoring and Assessment*, *195*(5). https://doi.org/10.1007/s10661-023-11245-2
...
Zurrell, D. (2020). *Introduction to species distribution modelling (SDM) in R \[Software\]*. GitHub. https://damariszurell.github.io/SDM-Intro/
